...
from tests.test_utils.mock_entities import MOCK_FILTERS
...
class TestOrderRepository(unittest.TestCase):
    ...
    def test_update_order_statuses_massively_returns_updated_orders(self):
        ...
        self.test_db.session.query.return_value.filter.return_value.options.return_value.all.return_value = [order_from_db]
    ...
    def test_update_order_returns_updated_order(self):
        ...
        self.test_db.session.query.return_value.filter_by.return_value.options.return_value.first.return_value = order_from_db
    ...
    def test_update_order_throws_exception_when_order_cannot_be_found(self):
        ...
        self.test_db.session.query.return_value.filter_by.return_value.options.return_value.first.return_value = None
    ...
    @patch('my_app.api.repositories.order_repository.paginate')
    def test_get_orders_of_printer_returns_order_page(self, paginate_mock):
        paginate_mock.return_value.all.return_value = [MOCK_ORDER_MODEL]

        response = self.order_repository.get_orders_of_printer(1, MOCK_FILTERS)
