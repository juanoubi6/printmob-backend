...
from tests.test_utils.mock_entities import MOCK_BUYER, MOCK_PRINTER, MOCK_BALANCE, MOCK_DESIGNER, \
    MOCK_PRINTER_DATA_DASHBOARD, MOCK_DESIGNER_DATA_DASHBOARD, MOCK_BUYER_DATA_DASHBOARD
from tests.test_utils.test_json import GET_PRINTER_PROFILE_RESPONSE_JSON, GET_BUYER_PROFILE_RESPONSE_JSON, \
    ...
    GET_DESIGNER_PROFILE_RESPONSE_JSON, CREATE_DESIGNER_JSON_REQUEST, PRINTER_DATA_DASHBOARD_RESPONSE_JSON, \
    DESIGNER_DATA_DASHBOARD_RESPONSE_JSON, BUYER_DATA_DASHBOARD_RESPONSE_JSON
...
class TestUserController(unittest.TestCase):
    ...
    @patch.object(app.user_controller, "user_service")
    def test_ger_user_profile_returns_designer_data_when_designer_token_is_provided(self, mock_user_service):
        mock_user_service.get_designer_by_email.return_value = MOCK_DESIGNER

        res = client.get(
            "/users/{}/profile".format(MOCK_DESIGNER.id),
            headers={"Authorization": self.designer_authorization_token}
        )

        assert res.status_code == 200
        assert res.json == GET_DESIGNER_PROFILE_RESPONSE_JSON

        mock_user_service.get_designer_by_email.assert_called_once_with(MOCK_DESIGNER.email)
    ...
    @patch.object(app.user_controller, "user_service")
    def test_update_user_profile_returns_updated_designer_data_when_designer_token_is_provided(self, mock_user_service):
        mock_user_service.update_designer.return_value = MOCK_DESIGNER

        res = client.put(
            "/users/{}/profile".format(MOCK_DESIGNER.id),
            headers={"Authorization": self.designer_authorization_token},
            data=json.dumps(CREATE_DESIGNER_JSON_REQUEST)
        )

        assert res.status_code == 200
        assert res.json == GET_DESIGNER_PROFILE_RESPONSE_JSON

        mock_user_service.update_designer.assert_called_once()
    ...
    @patch.object(app.user_controller, "user_service")
    def test_ger_user_balance_returns_balance_for_printer(self, mock_user_service):
    ...
    @patch.object(app.user_controller, "user_service")
    def test_ger_user_balance_returns_balance_for_designer(self, mock_user_service):
        mock_user_service.get_user_balance.return_value = MOCK_BALANCE

        res = client.get(
            "/users/{}/balance".format(MOCK_DESIGNER.id),
            headers={"Authorization": self.designer_authorization_token}
        )
    ...
    @patch.object(app.user_controller, "user_service")
    def test_request_user_balance_returns_ok_for_printer(self, mock_user_service):
        res = client.post(
            "/users/{}/balance".format(MOCK_PRINTER.id),
            headers={"Authorization": self.printer_authorization_token}
        )
    ...
    @patch.object(app.user_controller, "user_service")
    def test_request_user_balance_returns_ok_for_designer(self, mock_user_service):
        res = client.post(
            "/users/{}/balance".format(MOCK_DESIGNER.id),
            headers={"Authorization": self.designer_authorization_token}
        )
    ...
    @patch.object(app.user_controller, "user_service")
    def test_get_user_data_dashboard_returns_designer_dashboard_data_when_designer_token_is_provided(self, mock_user_service):
        mock_user_service.get_designer_data_dashboard.return_value = MOCK_DESIGNER_DATA_DASHBOARD

        res = client.get(
            "/users/{}/data-dashboard".format(MOCK_DESIGNER.id),
            headers={"Authorization": self.designer_authorization_token}
        )

        assert res.status_code == 200
        assert res.json == DESIGNER_DATA_DASHBOARD_RESPONSE_JSON

        mock_user_service.get_designer_data_dashboard.assert_called_once_with(MOCK_DESIGNER.id)

    @patch.object(app.user_controller, "user_service")
    def test_get_user_data_dashboard_returns_buyer_dashboard_data_when_designer_token_is_provided(self, mock_user_service):
        mock_user_service.get_buyer_data_dashboard.return_value = MOCK_BUYER_DATA_DASHBOARD

        res = client.get(
            "/users/{}/data-dashboard".format(MOCK_BUYER.id),
            headers={"Authorization": self.buyer_authorization_token}
        )

        assert res.status_code == 200
        assert res.json == BUYER_DATA_DASHBOARD_RESPONSE_JSON

        mock_user_service.get_buyer_data_dashboard.assert_called_once_with(MOCK_BUYER.id)
