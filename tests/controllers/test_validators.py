...
def test_validate_campaign_interval_works_with_valid_values():
    start_date = datetime.datetime.now() + datetime.timedelta(days=1)
    end_date = datetime.datetime.now() + datetime.timedelta(days=2)
    validate_campaign_time_interval(start_date, end_date)


def test_validate_campaign_interval_throws_exception_with_none_start_date():
    end_date = datetime.datetime.now() + datetime.timedelta(days=2)
    with pytest.raises(InvalidParamException):
        validate_campaign_time_interval(None, end_date)


def test_validate_campaign_interval_throws_exception_with_past_start_date():
    start_date = datetime.datetime.now() + datetime.timedelta(days=-1)
    end_date = datetime.datetime.now() + datetime.timedelta(days=2)
    with pytest.raises(InvalidParamException):
        validate_campaign_time_interval(start_date, end_date)
...
def test_validate_campaign_interval_throws_exception_with_none_end_date():
    start_date = datetime.datetime.now() + datetime.timedelta(days=1)
    with pytest.raises(InvalidParamException):
        validate_campaign_time_interval(start_date, None)


def test_validate_campaign_interval_throws_exception_with_past_end_date():
    start_date = datetime.datetime.now() + datetime.timedelta(days=1)
    end_date = datetime.datetime.now() + datetime.timedelta(days=-2)
    with pytest.raises(InvalidParamException):
        validate_campaign_time_interval(start_date, end_date)
...
def test_validate_campaign_interval_throws_exception_with_invalid_interval():
